...
type RuntimeInfoStore struct {
	sync.RWMutex
	baseIDToKouplelessNode map[string]*node_provider.KouplelessNode
...
func NewRuntimeInfoStore() *RuntimeInfoStore {
	return &RuntimeInfoStore{
		...
		baseIDToKouplelessNode: make(map[string]*node_provider.KouplelessNode),
		baseLatestMsgTime:      make(map[string]int64),
		baseIDLock:             make(map[string]bool),
	}
}

func (r *RuntimeInfoStore) PutKouplelessNode(baseID string, k *node_provider.KouplelessNode) {
	r.Lock()
	defer r.Unlock()
...
func (r *RuntimeInfoStore) GetKouplelessNode(baseID string) *node_provider.KouplelessNode {
	r.RLock()
	defer r.RUnlock()
	return r.baseIDToKouplelessNode[baseID]
}

func (r *RuntimeInfoStore) GetKouplelessNodes() []*node_provider.KouplelessNode {
	r.RLock()
	defer r.RUnlock()
	ret := make([]*node_provider.KouplelessNode, 0)
...
func (r *RuntimeInfoStore) getBaseIDFromNodeID(nodeID string) string {
	if !strings.HasPrefix(nodeID, node_provider.VIRTUAL_NODE_NAME_PREFIX) {
		return ""
	}
	return nodeID[len(node_provider.VIRTUAL_NODE_NAME_PREFIX):]
}

func (r *RuntimeInfoStore) GetBaseNodeByNodeID(nodeID string) *node_provider.KouplelessNode {
	r.Lock()
	defer r.Unlock()
