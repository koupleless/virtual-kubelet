...
func (brc *BaseRegisterController) Run(ctx context.Context) {
	...
	brc.config.MqttConfig.OnConnectHandler = func(client paho.Client) {
		...
		client.Subscribe(model.BaseHeartBeatTopic, mqtt.Qos1, brc.heartBeatMsgCallback)
		client.Subscribe(model.BaseHealthTopic, mqtt.Qos1, brc.healthMsgCallback)
		client.Subscribe(model.BaseBizTopic, mqtt.Qos1, brc.bizMsgCallback)
		select {
		case <-brc.ready:
		default:
			close(brc.ready)
...
func (brc *BaseRegisterController) startVirtualKubelet(baseID string, initData model.HeartBeatData) {
	...
	kn, err := node_provider.NewKouplelessNode(&model.BuildKouplelessNodeConfig{
