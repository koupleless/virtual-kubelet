	...
	var eventHandler cache.ResourceEventHandler = cache.ResourceEventHandlerFuncs{
		AddFunc: func(pod interface{}) {
			...
			if key, err := cache.MetaNamespaceKeyFunc(pod); err != nil {
				...
				sys.pc.StorePod(key, nil)
				sys.pc.SyncPodsFromKubernetesEnqueue(ctx, key)
			}
		},
		UpdateFunc: func(oldObj, newObj interface{}) {
			...
			if key, err := cache.MetaNamespaceKeyFunc(newPod); err != nil {
				...
				if kPod.lastPodStatusUpdateSkipped &&
					...
					sys.pc.SyncPodStatusFromProviderEnqueue(ctx, key)
		...
		DeleteFunc: func(pod interface{}) {
			...
			if key, err := cache.DeletionHandlingMetaNamespaceKeyFunc(pod); err != nil {
				...
				ctx = span.WithField(ctx, "key", key)
				sys.pc.DeletePod(key)
				sys.pc.SyncPodsFromKubernetesEnqueue(ctx, key)
				// If this pod was in the deletion queue, forget about it
				key = fmt.Sprintf("%v/%v", key, k8sPod.UID)
				sys.pc.DeletePodsFromKubernetesForget(ctx, key)
			}
		},
	}

	_, err = sys.pc.informer.Informer().AddEventHandler(eventHandler)
	if err != nil {
		log.G(ctx).Error(err)
	}
