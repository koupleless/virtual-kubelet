...
type Node struct {
	nc *vnode.NodeController
	pc *vnode.PodController
...
func (n *Node) NodeController() *vnode.NodeController {
	return n.nc
}

// PodController returns the configured pod controller.
func (n *Node) PodController() *vnode.PodController {
	return n.pc
}
...
type NodeConfig struct {
	...
	NodeStatusUpdateErrorHandler vnode.ErrorHandler
}
...
func NewNode(name string, newProvider NewProviderFunc, opts ...NodeOpt) (*Node, error) {
	...
	if np == nil {
		nnp := vnode.NewNaiveNodeProvider()
		np = nnp
	...
	nodeControllerOpts := []vnode.NodeControllerOpt{
		vnode.WithNodeEnableLeaseV1(NodeLeaseV1Client(cfg.Client), vnode.DefaultLeaseDuration),
	}

	if cfg.NodeStatusUpdateErrorHandler != nil {
		nodeControllerOpts = append(nodeControllerOpts, vnode.WithNodeStatusUpdateErrorHandler(cfg.NodeStatusUpdateErrorHandler))
	}

	nc, err := vnode.NewNodeController(
		np,
		&cfg.NodeSpec,
	...
	pc, err := vnode.NewPodController(vnode.PodControllerConfig{
