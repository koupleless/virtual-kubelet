...
func (t *DefaultTracker) Eventually(traceID, scene, event string, labels map[string]string, timeoutErrorCode model.ErrorCode, checkFunc func(ctx context.Context) (bool, error), timeout time.Duration, interval time.Duration, checkPassed func(), timeoutCall func()) {
	startTime := time.Now()
	data := logData{
		TraceID: traceID,
		Scene:   scene,
